...
app.get("/", async (req, res) => {
  try {
    ...
    if (hiddenAuthorIds.length > 0) filter.createdBy = { $nin: hiddenAuthorIds };
    ...
    const [blogs, totalBlogs, featuredBlogs] = await Promise.all([
      Blog.find(filter).sort(sortOption).skip(skip).limit(parseInt(limit)).populate("createdBy", "fullName profileImageURL").lean(),
      Blog.countDocuments(filter),
      Blog.find({ isFeatured: true, status: "published", isDeleted: false, createdBy: { $nin: hiddenAuthorIds } }).sort({ featuredRank: 1 }).limit(3).populate("createdBy", "fullName profileImageURL").lean()
    ]);
...
// Public auth aliases. Render directly so /signin and /signup do not depend on a redirect hop.
